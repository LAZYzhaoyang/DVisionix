...
- `DetectionTask`：中心点目标分配（GT 框中心落入的单元为正样本），
...
## 数据格式
检测样本需要 `image_path` + `boxes` + `labels`，坐标为像素 `[x1, y1, x2, y2]`：
...
samples = [
    {"image_path": "img1.png", "boxes": [[5, 5, 25, 25]], "labels": [0]},
    {"image_path": "img2.png", "boxes": [[3, 3, 30, 30], [40, 40, 60, 60]], "labels": [1, 2]},
]
ds = CustomDataset(task_type="detection", samples=samples, num_classes=3)
...
```bash
conda run -n dvisionix python demos/train_detection.py
```

由 `configs/detection/demo_synthetic.yaml` 驱动，使用**可学习的**合成数据
（按类别绘制实心矩形）。demo 会在训练后自动评估 mAP。
...
- 损失权重可调：`DetectionTask(obj_weight=1.0, box_weight=5.0, cls_weight=1.0)`。
