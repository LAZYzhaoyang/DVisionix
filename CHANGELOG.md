...
阶段 0-5 全部完成：装配层门禁 → 修复跑不通的官方配置 → P0 正确性 →
训练/评估/导出闭环 → 打包与 CI → 性能优化。
20 项已核实缺陷 D1-D20 全部修复并附回归测试；官方配置由 14/17 可跑提升到 **17/17**。

> 会改变训练结果、输出格式或 API 行为的变更，见
> [docs/v1.1_changes.md](docs/v1.1_changes.md)（迁移指南）。
